...
PUBLIC Handler, Port, n, Se
    ...
    * Si la impresora fiscal estuviese configurada a 19200, esta funcion fija
	* la velocidad del puerto serie de la PC en 19200.
	* -----------------------------------------------------------------------
	* SetBaud (Handler, 19200)

	* Establecer Protocolo con consulta de estado intermedio ( STATPRN ). 
	* NO valido en otros modelos de impresoras fiscales.
	NProtocol(1)
...
* El cuerpo es valido cualquiera sea el Documento a emitir ( Fiscal o Nota de
* Credito ).
*----------------------------------------------------------------------------
...
* cando Error.
* Consultar: drivers.doc para el tratamiento de estos errores
* ------------------------------------------------------------------------
...
IF ( StatPrn < 0 )
	DO WHILE StatPrn = -9
		...
		StatPrn = MandaPaq (Handler, String)
		IF LASTKEY() = 27
			? "Fin consulta de estado intermedio"
			RETURN -9
		ENDIF
		inkey (1)
	ENDDO
